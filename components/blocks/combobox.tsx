...
function Combobox({
  ...
  const [inputValue, setInputValue] = useControllableState({
    ...
    defaultValue:
      defaultInputValue ?? getItemByValue(normalizedItems, defaultValue)?.label ?? "",
  ...
  const updateInputValue = React.useCallback(
    ...
    [filter, normalizedItems, selectedItem, setInputValue, updateHighlightedValue]
  )
...
function ComboboxInput({
  ...
  const hasClearValue = combobox.inputValue.length > 0 || !!combobox.selectedItem
...
function ComboboxList({
  ...
  children?:
    | React.ReactNode
    | ((item: ComboboxItemData) => React.ReactNode)
...
function ComboboxItem({
  ...
  const combobox = useCombobox()
  const resolvedItem =
    item ?? getItemByValue(combobox.items, value) ?? {
      value: value ?? "",
      label: value ?? "",
    }
...
export {
    ...
    ComboboxItem,
    ComboboxList,
    ComboboxTrigger, useCombobox, type ComboboxItemData,
